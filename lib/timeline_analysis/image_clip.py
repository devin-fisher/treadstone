...
from lib.image_generator.image_build import build_infographics
import requests
import json
...
def create_image_video(game_clip_dir, game_data):
    infographic_data = game_data[time_line_infographic]
    infographic_list = []
    for b in range(0, len(infographic_data)):
        infographic_image = build_info_graphics(infographic_data)
        infographic_time = infographic_data[b]['timeStamp']
        audio = AudioFileClip("E:\Youtube\EU_Spring_2017\Week1\MSFvsGIA\silence.wav")
        clip = ImageClip(infographic_image,duration=3).set_audio(audio).fadein(1)
        clip.write_videofile(path + infographic_time + ".mp4",fps=60, codec='libx264',
             audio_codec='aac',
             temp_audiofile='temp-audio.m4a',
             remove_temp=True)
        infographic_list.append(infographic_time)
...
def create_sub_clips(path, game_number, event_translation):
    for a in range(0, len(event_translation)):
        ...
        #
        subprocess.call(['ffmpeg', '-y', '-ss', str(ss), '-i', path + input_vid, '-t', str(duration), '-vf', 'fade=in:st=0:d=2,fade=out:st=' + str(fade_end) + ':d=2', '-af', 'afade=t=in:ss=0:d=3,afade=t=out:st=' + str(fade_end) + ':d=2',  '-preset',  'ultrafast',  '-avoid_negative_ts', 'make_zero', '-fflags', '+genpts', path + clip_name + '.mp4'])
    ts_copies = make_ts_copies(path, game_data)
    concatenate(path, ts_copies)



def make_ts_copies(path, game_data):
    time_before = 0
    infographic_data = game_data[time_line_infographic]
    translation_data = game_data[translations] #ask devin for the real location for this data
    for a in range(0, len(event_translation)):
        start_time = translation_data[a]['video_start']
        game_time = translation_data[a]['game_end']
        for b in range(0, len(report_info['infographics'])):
            infographic = infographic_data[b]['timeStamp']
...
def concatenate(path, ts_copies):
